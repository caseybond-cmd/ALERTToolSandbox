...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeApp() {
        ...
        const savedState = localStorage.getItem('alertToolState_v25');
        if (savedState) {
    ...
    function saveState() {
        ...
        localStorage.setItem('alertToolState_v25', JSON.stringify(currentReview));
    }
    ...
    function clearForm(clearStorage = true) {
        ...
        deviceCounters = {};
        if (clearStorage) {
            localStorage.removeItem('alertToolState_v25');
            currentReview = {};
        }
        form.querySelectorAll('select.score-input, .score-note').forEach(el => {
    ...
    function generateClinicalInsights(data, bloodInsights) {
        ...
        if (parseInt(data.age, 10) > 65 && parseInt(data.severe_comorbidities, 10) >= 2) insights.push(createInsight('<b>High-Risk Profile:</b> Patient has multiple baseline risk factors.', 'medium'));
        ...
        if (bloodInsights.lactate.score > 0) insights.push(createInsight('<b>Metabolic Stress/Hypoperfusion:</b> Elevated lactate is a critical finding requiring urgent escalation.', 'high'));
        ...
        if (parseInt(data.delirium, 10) > 0 && parseInt(data.frailty, 10) > 0) insights.push(createInsight('<b>Neurological Vulnerability:</b> Delirium in a frail patient is a major predictor of poor outcomes.', 'high'));
...
ALERT NURSE REVIEW
------------------
PATIENT: ${data.patient_initials || ''}-${data.patient_urn_last4 || ''} | Age: ${data.age || 'N/A'} | Ward: ${data.ward === 'Other' ? data.wardOther : data.ward || 'N/A'}-${data.room_number || 'N/A'}

FINAL SCORE: ${score} (${category})
------------------
CLINICAL INSIGHTS IDENTIFIED:
...
- Breathing: RR ${data.rr_input || 'N/A'}, SpO2 ${data.spo2_input || 'N/A'}% on ${data.o2_device_input} (${data.o2_flow_input || 'N/A'}L, ${data.fio2_input || 'N/A'}%, PEEP ${data.peep_input || 'N/A'}, PS ${data.ps_input || 'N/A'})
- Circulation: HR ${data.hr_input || 'N/A'}, SBP ${data.sbp_input || 'N/A'}
...
`;
    ...
    function setupEventListeners() {
        ...
        document.getElementById('assessment-container').addEventListener('change', e => {
            ...
            if (e.target.id === 'o2_device_input') {
                const device = e.target.value;
                document.getElementById('o2_flow_container').style.display = device === 'NP' ? 'block' : 'none';
                document.getElementById('o2_fio2_container').style.display = ['HFNP', 'NIV'].includes(device) ? 'block' : 'none';
                document.getElementById('o2_niv_container').style.display = device === 'NIV' ? 'block' : 'none';
            }
        });

        form.addEventListener('change', e => {
            if (e.target.classList.contains('score-input')) {
                const noteBox = e.target.closest('div').querySelector('.score-note');
                if(noteBox) {
                    const show = (e.target.type === 'checkbox' && e.target.checked && e.target.dataset.score !== '0') || 
                                 (e.target.tagName === 'SELECT' && e.target.value !== '0');
                    noteBox.style.display = show ? 'block' : 'none';
                }
            }
        });
    }
    ...
    function populateStaticContent() {
        const createBloodInput = (label, id, unit, trend=true) => `<div class="flex items-center gap-x-2 blood-score-item"><label class="block text-sm font-medium w-32">${label}</label><div class="flex-grow grid ${trend ? 'grid-cols-2' : 'grid-cols-1'} gap-x-2"><input type="number" step="0.1" id="${id}" class="w-full rounded-md border-2 p-2 text-sm" placeholder="Current">${trend ? `<input type="number" step="0.1" id="${id}_prev" class="w-full rounded-md border-2 p-2 text-sm" placeholder="Previous">` : ''}</div><span class="text-xs text-gray-500 w-16">${unit}</span><span id="${id}_blood_score" class="blood-score-badge score-0">+0</span></div>`;
        document.getElementById('bloods-container').innerHTML = `<h2 class="text-xl font-bold border-b pb-3 mb-4">Bloods</h2><div class="space-y-3">${createBloodInput('Creatinine', 'creatinine', 'µmol/L')}<div class="flex items-center gap-x-2 blood-score-item"><label class="block text-sm font-medium w-32">Neutrophils / Lymphocytes</label><div class="flex-grow grid grid-cols-2 gap-x-2"><input type="number" step="0.1" id="neutrophils" class="w-full rounded-md border-2 p-2 text-sm" placeholder="Neutrophils"><input type="number" step="0.1" id="lymphocytes" class="w-full rounded-md border-2 p-2 text-sm" placeholder="Lymphocytes"></div><span class="text-xs text-gray-500 w-16"></span><span id="nlr_blood_score" class="blood-score-badge score-0">+0</span></div>${createBloodInput('Albumin', 'albumin', 'g/L', false)}${createBloodInput('RDW', 'rdw', '%')}${createBloodInput('Lactate', 'lactate', 'mmol/L', false)}${createBloodInput('Bilirubin', 'bilirubin', 'µmol/L')}${createBloodInput('Platelets', 'platelets', 'x10⁹/L')}${createBloodInput('Hemoglobin', 'hemoglobin', 'g/dL', false)}${createBloodInput('Glucose', 'glucose', 'mg/dL', false)}</div><div class="mt-4 pt-4 border-t text-right font-bold text-lg">Total Blood Score: <span id="total_blood_score" class="text-teal-600">0</span></div> <div class="mt-6 pt-4 border-t"><h3 class="font-semibold text-gray-700 mb-2">Key Electrolytes (Non-Scored)</h3><div class="grid sm:grid-cols-2 gap-x-6 gap-y-4"><div><label>K+</label><input type="number" step="0.1" id="k_input" class="mt-1 w-full rounded-md border-2 p-2"><div class="flex gap-x-2 mt-1"><label class="text-xs flex items-center"><input type="checkbox" id="k_replaced" class="mr-1">Replaced</label><label class="text-xs flex items-center"><input type="checkbox" id="k_planned" class="mr-1">Planned</label></div></div><div><label>Mg++</label><input type="number" step="0.1" id="mg_input" class="mt-1 w-full rounded-md border-2 p-2"><div class="flex gap-x-2 mt-1"><label class="text-xs flex items-center"><input type="checkbox" id="mg_replaced" class="mr-1">Replaced</label><label class="text-xs flex items-center"><input type="checkbox" id="mg_planned" class="mr-1">Planned</label></div></div></div></div>`;
        document.getElementById('assessment-container').innerHTML = `<h2 class="text-xl font-bold border-b pb-3 mb-4">A-E Assessment & ADDS</h2><div class="space-y-6"><div><h3 class="assessment-section-title">A: Airway</h3><select id="airway_input" class="w-full rounded-md border-2 p-2"><option value="Clear">Clear and maintained</option><option value="At Risk">Airway at risk / requires adjunct</option></select></div><div><h3 class="assessment-section-title">B: Breathing</h3><div class="assessment-grid"><label>O₂ Device:<select id="o2_device_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"><option value="RA">Room Air</option><option value="NP">Nasal Prongs</option><option value="HFNP">High-Flow</option><option value="NIV">NIV/CPAP</option></select></label><div id="o2_flow_container" style="display:none;"><label>Flow (L/min):<input type="number" id="o2_flow_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label></div><div id="o2_fio2_container" style="display:none;"><label>FiO2 (%):<input type="number" id="fio2_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label></div><div id="o2_niv_container" style="display:none;" class="grid grid-cols-2 gap-2"><label>PEEP:<input type="number" id="peep_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label><label>PS:<input type="number" id="ps_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label></div><label>Resp Rate:<input type="number" id="rr_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label><label>SpO2 (%):<input type="number" id="spo2_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label></div></div><div><h3 class="assessment-section-title">C: Circulation</h3><div class="assessment-grid"><label>Heart Rate:<input type="number" id="hr_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label><label>Systolic BP:<input type="number" id="sbp_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label></div></div><div><h3 class="assessment-section-title">D: Disability</h3><select id="consciousness_input" class="vital-input w-full rounded-md border-2 p-2"><option>Alert</option><option>Voice</option><option>Pain</option><option>Unresponsive</option></select></div><div><h3 class="assessment-section-title">E: Exposure</h3><label>Temperature (°C):<input type="number" step="0.1" id="temp_input" class="vital-input mt-1 w-full rounded-md border-2 p-2"></label></div></div><div class="mt-6 bg-gray-100 p-4 rounded-lg border"><label class="flex items-center"><input type="checkbox" id="addsModificationCheckbox"> <span class="ml-2 text-sm font-medium">Apply MODS to ADDS</span></label><div id="addsModificationDetails" class="hidden ml-6 mt-4 space-y-4"><div><label class="block text-sm font-medium">Manual Override ADDS Score:</label><input type="number" id="manualADDSScore" class="mt-1 w-full rounded-md border-2 p-2"></div><div><label class="block text-sm font-medium">Rationale:</label><textarea id="addsModificationText" rows="2" class="mt-1 w-full rounded-md border-2 p-2"></textarea></div></div></div><div class="mt-6 grid grid-cols-2 gap-4 text-center"><div class="bg-blue-50 p-4 rounded-lg border border-blue-200"><span class="text-sm font-medium text-gray-500">CALCULATED ADDS</span><div id="calculatedADDSScore" class="font-bold text-5xl my-2">0</div></div><div class="bg-teal-50 p-4 rounded-lg border border-teal-200"><span class="text-sm font-medium text-gray-500">FINAL ADDS SCORE</span><div id="finalADDSScore" class="font-bold text-5xl my-2">0</div></div></div>`;
        const buildNote = (id) => `<textarea id="${id}_note" class="score-note mt-2 w-full p-2 text-sm border rounded hidden" placeholder="Add details..."></textarea>`;
        document.getElementById('scoringContainer').innerHTML = `<h2 class="text-xl font-bold border-b pb-3 mb-4">Clinical Risk Factors</h2>
            <div class="space-y-2">
                <div><label class="block font-medium mb-1">Baseline Patient Factors:</label><div class="pl-2 border-l-2 space-y-2"><label>Age:<input type="number" id="age" class="score-input mt-1 w-full rounded-md border-2 p-2"></label><label>Admission Type:<select id="admission_type" class="score-input w-full rounded-md border-2 p-2"><option value="0">Surgical (Elective)</option><option value="1">Surgical (Emergency)</option><option value="2">Medical or ED</option></select></label><label>ICU LOS (days):<input type="number" id="los_days" class="score-input mt-1 w-full rounded-md border-2 p-2"></label><label>Severe Comorbidities (No.):<input type="number" id="severe_comorbidities" class="score-input mt-1 w-full rounded-md border-2 p-2"></label></div></div>
                <div><label class="list-score-option"><input type="checkbox" class="score-input" data-score="3" id="met_criteria"><span class="score-label">Patient is in MET Criteria</span><span class="score-value">+3</span></label>${buildNote('met_criteria')}</div>
                <div><label class="list-score-option"><input type="checkbox" class="score-input" data-score="2" id="increasing_o2"><span class="score-label">Increasing O₂ Trend</span><span class="score-value">+2</span></label>${buildNote('increasing_o2')}</div>
                <div><label class="list-score-option"><input type="checkbox" class="score-input" data-score="2" id="rapid_wean"><span class="score-label">Rapid Wean of Resp Support</span><span class="score-value">+2</span></label>${buildNote('rapid_wean')}</div>
                <div><label class="list-score-option"><input type="checkbox" class="score-input" data-score="1" id="high_risk_airway"><span class="score-label">High Risk Airway</span><span class="score-value">+1</span></label>${buildNote('high_risk_airway')}</div>
                <div><label class="list-score-option"><input type="checkbox" class="score-input" data-score="1" id="high_risk_drain"><span class="score-label">High Risk Drain</span><span class="score-value">+1</span></label>${buildNote('high_risk_drain')}</div>
                <div><label class="block font-medium mb-1">Pain Score:</label><select id="pain_score" class="score-input w-full rounded-md border-2 p-2"><option value="0">None / Well Controlled</option><option value="1">Significant / PRN Use</option><option value="2">High / PCA / APS Review</option></select>${buildNote('pain_score')}</div>
                <div><label class="block font-medium mb-1">Gastrointestinal:</label><div class="pl-2 border-l-2"><label class="flex items-center mt-2"><input type="checkbox" id="gi_nbm" class="score-input mr-2" data-score="2">NBM/NG/TPN</label><label class="flex items-center mt-2"><input type="checkbox" id="gi_ileus" class="score-input mr-2" data-score="1">Ileus / Bowels not opened >3 days</label></div></div>
                <div><label class="block font-medium mb-1">Delirium:</label><select id="delirium" class="score-input w-full rounded-md border-2 p-2"><option value="0">None</option><option value="1">Mild</option><option value="2">Moderate-Severe</option></select>${buildNote('delirium')}</div>
                <div><label class="block font-medium mb-1">Mobility:</label><select id="mobility" class="score-input w-full rounded-md border-2 p-2"><option value="0">Baseline</option><option value="0">Limited due to lines/attachments</option><option value="1">Assisted</option><option value="2">Bed-bound</option></select>${buildNote('mobility')}</div>
                <div><label class="block font-medium mb-1">Frailty (Pre-hospital):</label><select id="frailty" class="score-input w-full rounded-md border-2 p-2"><option value="0">Not Frail</option><option value="1">Mild</option><option value="2">Moderate-Severe</option></select>${buildNote('frailty')}</div>
                <div><label class="block font-medium mb-1">Staffing (Reducer):</label><select id="staffing" class="score-input w-full rounded-md border-2 p-2"><option value="0">Standard Ratio</option><option value="-1">Enhanced Care (1:1, 1:2)</option></select>${buildNote('staffing')}</div>
            </div>`;
